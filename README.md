...
* The user also has the ability to toggle real time updates on/off, in the upper right corner. This will center the map on the user’s current location, and also update the directions to reach his/her desired destination. With the real time button selected, these updates will occur automatically. This feature can be toggled off mid workout if the user desires to switch destination.
